...
function ShareEvent({ event }: { event: EventItem }) {
  ...
  const recordShare = (channel: string) => { try { const saved = JSON.parse(localStorage.getItem("eventpulse-referral-insights-v1") ?? "{}"); const current = saved[referralId] ?? { title: event.title, shares: 0, clicks: 0, channels: {} }; current.shares += 1; current.channels[channel] = (current.channels[channel] ?? 0) + 1; saved[referralId] = current; localStorage.setItem("eventpulse-referral-insights-v1", JSON.stringify(saved)); } catch { /* Sharing remains available if storage is unavailable. */ } };
