...
function ShareEvent({ event }: { event: EventItem }) {
  ...
  const url = typeof window !== "undefined" ? `${window.location.origin}/?event=${event.id}&ref=${referralId}` : `/?event=${event.id}&ref=${referralId}`;
  const message = `${event.title} · ${event.date} at ${event.venue}`;
  const recordShare = (channel: string) => { try { const saved = JSON.parse(localStorage.getItem("eventpulse-referral-insights-v1") ?? "{}"); const current = saved[referralId] ?? { title: event.title, shares: 0, clicks: 0, channels: {}, trend: {} }; const day = new Date().toISOString().slice(0, 10); current.trend ??= {}; current.trend[day] ??= { shares: 0, clicks: 0 }; current.shares += 1; current.trend[day].shares += 1; current.channels[channel] = (current.channels[channel] ?? 0) + 1; saved[referralId] = current; localStorage.setItem("eventpulse-referral-insights-v1", JSON.stringify(saved)); window.dispatchEvent(new Event("eventpulse:referral-updated")); } catch { /* Sharing remains available if storage is unavailable. */ } };
...
export default function Home() {
  ...
  useEffect(() => { const params = new URLSearchParams(window.location.search); const referral = params.get("ref"); if (!referral) return; try { const saved = JSON.parse(localStorage.getItem("eventpulse-referral-insights-v1") ?? "{}"); const current = saved[referral] ?? { title: "Shared EventPulse event", shares: 0, clicks: 0, channels: {}, trend: {} }; const day = new Date().toISOString().slice(0, 10); current.trend ??= {}; current.trend[day] ??= { shares: 0, clicks: 0 }; current.clicks += 1; current.trend[day].clicks += 1; saved[referral] = current; localStorage.setItem("eventpulse-referral-insights-v1", JSON.stringify(saved)); window.dispatchEvent(new Event("eventpulse:referral-updated")); } catch { /* The shared event remains available if local analytics is unavailable. */ } }, []);
